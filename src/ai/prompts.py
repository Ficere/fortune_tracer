...
def build_full_analysis_prompt(bazi_json: str) -> str:
    """构建综合分析提示词（整合所有测算结果）"""
    return f"""请根据以下完整命理数据进行深度综合分析：
...
## 分析要求
请综合以上所有数据，从以下维度进行专业解读（每维度200-300字）：

1. **性格特点**：结合日主五行、十神格局、神煞分析性格特征
2. **事业运势**：结合喜用神、格局、大运分析事业发展
3. **感情运势**：结合纳音、神煞、宫位分析感情模式
4. **健康建议**：结合五行偏弱、神煞给出养生建议
5. **财运分析**：结合称骨、喜用神、大运分析财运特点
6. **综合评价**：整体命局评价和人生建议
